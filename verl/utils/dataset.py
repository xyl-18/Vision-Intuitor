...
def _infer_file_type_from_dir(data_path: str) -> str:
    # Prefer known dataset payload files; ignore metadata files when mixed.
    supported = ("parquet", "json", "jsonl", "csv")
    files = [p for p in Path(data_path).iterdir() if p.is_file()]
    if not files:
        raise ValueError(f"No data files found in directory: {data_path}")

    exts = [_normalize_file_type(p.suffix) for p in files if _normalize_file_type(p.suffix) in supported]
    if not exts:
        raise ValueError(
            f"No supported data files found in directory: {data_path}. "
            f"Supported extensions: {supported}"
        )

    # If parquet exists, use parquet to avoid accidentally selecting metadata json.
    if "parquet" in exts:
        return "parquet"

    # Fallback to the most common extension among supported files.
...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        if os.path.isdir(data_path):
            ...
            file_type = _infer_file_type_from_dir(data_path)
