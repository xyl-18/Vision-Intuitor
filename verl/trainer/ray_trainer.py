...
from .intuitor_mixer import IntuitorAuxRewardMixer
from .metrics import (
...
def compute_advantage(data: DataProto, adv_estimator: AdvantageEstimator, gamma: float = 1.0, lam: float = 1.0):
    ...
    if "intuitor_internal_token_level_scores" in data.batch:
        adv_inputs["intuitor_internal_token_level_scores"] = data.batch["intuitor_internal_token_level_scores"]

    if "intuitor_format_token_level_scores" in data.batch:
        adv_inputs["intuitor_format_token_level_scores"] = data.batch["intuitor_format_token_level_scores"]

    if "intuitor_external_token_level_scores" in data.batch:
        adv_inputs["intuitor_external_token_level_scores"] = data.batch["intuitor_external_token_level_scores"]

    if "intuitor_length_token_level_scores" in data.batch:
        adv_inputs["intuitor_length_token_level_scores"] = data.batch["intuitor_length_token_level_scores"]

    if "intuitor_weight" in data.meta_info:
        adv_inputs["intuitor_weight"] = data.meta_info["intuitor_weight"]
    if "format_weight" in data.meta_info:
        adv_inputs["format_weight"] = data.meta_info["format_weight"]
    if "external_weight" in data.meta_info:
...
def _update_score_stats(metrics: dict[str, Any], prefix: str, values: torch.Tensor) -> None:
...
def _log_intuitor_reward_metrics(
    ...
    """Log Intuitor component rewards and weighted intuitor_overall for monitoring."""
    internal_token_scores = batch.batch["intuitor_internal_token_level_scores"].detach()

    mask = batch.batch["response_mask"].to(internal_token_scores.dtype)
    intuitor_scores = VF.masked_mean(internal_token_scores, mask=mask, dim=-1)
    _update_score_stats(metrics, "reward_component/intuitor", intuitor_scores)

    format_scores = torch.zeros_like(intuitor_scores)
    if mixer.format_weight > 0.0 and "format" in reward_metrics:
        format_scores = torch.as_tensor(
            reward_metrics["format"],
            dtype=intuitor_scores.dtype,
            device=intuitor_scores.device,
        )

    external_scores = torch.zeros_like(intuitor_scores)
    if mixer.external_weight > 0.0:
        if mixer.external_reward_key == "overall":
            if "overall" in reward_metrics:
                external_scores = torch.as_tensor(
                    reward_metrics["overall"],
                    dtype=intuitor_scores.dtype,
                    device=intuitor_scores.device,
                )
            else:
                external_scores = reward_tensor.sum(dim=-1).to(intuitor_scores.dtype)
        elif mixer.external_reward_key in reward_metrics:
            external_scores = torch.as_tensor(
                reward_metrics[mixer.external_reward_key],
                dtype=intuitor_scores.dtype,
                device=intuitor_scores.device,
            )

    length_scores = torch.zeros_like(intuitor_scores)
    if mixer.length_weight > 0.0:
        if "length" in reward_metrics:
            length_scores = torch.as_tensor(
                reward_metrics["length"],
                dtype=intuitor_scores.dtype,
                device=intuitor_scores.device,
            )

    intuitor_overall_scores = (
        batch.meta_info["intuitor_weight"] * intuitor_scores
        + batch.meta_info["format_weight"] * format_scores
        + batch.meta_info["external_weight"] * external_scores
...
def _log_response_entropy_metrics(metrics: dict[str, Any], batch: DataProto) -> None:
    """Log response entropy statistics from actor recomputation outputs."""
    if "entropys" not in batch.batch:
        return

    entropys = batch.batch["entropys"].detach()
    mask = batch.batch["response_mask"].to(entropys.dtype)
    response_entropy = VF.masked_mean(entropys, mask=mask, dim=-1)
    _update_score_stats(metrics, "actor/response_entropy", response_entropy)
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        # intuitor_aux_mixer builds token-level auxiliary scores for advantage fusion, 
        # which will be added to the original reward. It is compatible with all advantage estimators, 
        # and can be used as a general auxiliary reward mixer even outside of intuitor algorithm, 
        # while intuitor algorithm can also work without it by setting aux_weight=0.
        
        if config.algorithm.adv_estimator not in list(AdvantageEstimator):
            raise NotImplementedError(f"Unknown advantage estimator: {config.algorithm.adv_estimator}.")

        self.intuitor_aux_mixer = IntuitorAuxRewardMixer.from_algorithm_config(config.algorithm)
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                with timer("old", timing_raw):
                    ...
                    _log_response_entropy_metrics(metrics, batch)
                ...
                with timer("adv", timing_raw):
                    if "token_level_scores" not in batch.batch:
                        ...
                        if self.config.algorithm.adv_estimator == AdvantageEstimator.INTUITOR:
                            # Keep token_level_scores for unified metrics/logging.
                            # Intuitor optimization still uses self-certainty and auxiliary component advantages.
                            batch.batch["token_level_scores"] = reward_tensor
                            batch.meta_info["intuitor_weight"] = self.intuitor_aux_mixer.intuitor_weight
                            ...
                            if "format" in component_scores:
                                batch.batch["intuitor_format_token_level_scores"] = component_scores["format"]
                            if "external" in component_scores:
                                batch.batch["intuitor_external_token_level_scores"] = component_scores["external"]
                            if "length" in component_scores:
                                batch.batch["intuitor_length_token_level_scores"] = component_scores["length"]
                            metrics.update(aux_metrics)
                            _log_intuitor_reward_metrics(
                                metrics=metrics,
                                batch=batch,
                                reward_tensor=reward_tensor,
                                reward_metrics=reward_metrics,
