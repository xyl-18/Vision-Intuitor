...
# ## Instructions:
# Compare the answers with a semantic-first policy.

# Use a balanced policy:
# - Avoid false negatives: do not fail answers for harmless wording/format differences.
# - Avoid false positives: do not accept answers that conflict on core facts.

# Be flexible with non-semantic differences, including but not limited to:
# - Case differences (e.g., "Yes" vs "yes").
# - Minor formatting differences (spaces, punctuation, LaTeX wrappers, units/labels attached to equivalent value).
# - Equivalent expressions (e.g., "1/2" vs "0.5", "A" vs "(A)").
# - Simple word-form variants where meaning is unchanged (e.g., singular/plural, number word vs digit).
# - Common alias/descriptor variants that refer to the same specific entity.

# Core-fact consistency rules (strict):
# - If there is a direct conflict on key facts (entity identity, title/name, year/date, number/value, yes/no polarity, option letter, direction/location, disease/type), mark as incorrect.
# - If the model introduces major wrong facts that contradict ground truth, mark as incorrect even if some parts overlap.

# Important ground-truth syntax:
# - `<OR>` means any one of the listed options is acceptable.
# - `<AND>` means all listed key items must be present (order does not matter).

# Judging rules:
# - Allow concise exact answers even if ground truth uses expanded forms like `7<OR>=7`; answer `7` should be correct.
# - For open-ended questions, if the final answer is semantically consistent with ground truth, mark as correct.
# - For summary/report/introduction tasks, missing secondary details is acceptable if the main claims are correct and there is no core factual contradiction.
# - For identification tasks (person/movie/painting/disease/place/object), generic labels are insufficient when ground truth requires a specific identity.
# - Mark incorrect only for real semantic mismatch, missing required items under `<AND>`, or value/entity mismatch.
...
### Important Ground-Truth Syntax:
- `<OR>` indicates multiple alternative acceptable answers.
  - You MUST treat them as a list of candidates.
  - If the extracted answer matches ANY one candidate semantically, it is correct.
  - DO NOT interpret `<OR>` as a logical operator.

- `<AND>` indicates all listed key elements must be present.
  - The extracted answer must include ALL required items (order does not matter).

### Acceptable Variations:
- Case differences (e.g., "Yes" vs "yes").
- Minor formatting differences (spaces, punctuation, LaTeX wrappers).
- Equivalent expressions (e.g., "1/2" vs "0.5").
- Simple word-form variants (singular/plural, number word vs digit).
- Common aliases referring to the same specific entity.

### Strict Consistency Rules:
- If there is a conflict on key facts (entity identity, title/name, number, yes/no, location), mark as incorrect.
- If the answer identifies the WRONG specific entity (e.g., wrong movie, painting, person), mark as incorrect.
- If required elements under `<AND>` are missing → incorrect.

### Open-ended Tasks:
- Missing minor details is acceptable.
- The answer is correct if the main entity and key facts match without contradiction.
